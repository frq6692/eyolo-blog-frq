...
const Post = ({ post, text }: Props) => {
  return (
    <article
      className={`post-card post-category-top group relative ${
        text === "light" ? "has-line-link-white" : "has-line-link"
      }`}
    >
      <PostThumbnail post={post} text={text} />

      <div className={`mt-6 ${text === "light" ? "text-white" : "text-dark"}`}>
        ...
        <h3 className="text-2xl leading-relaxed mb-4 line-link line-clamp-3 text-inherit">
          <Link
            href={`/${post.slug}`}
        ...
        <span className="h-12 w-12 flex items-center justify-center text-[#90A096] group-hover:text-white bg-white/5 group-hover:bg-white/20 rounded-full has-transition group-hover:rotate-45">
          {/* prettier-ignore */}
          <svg width="14" height="14" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1.99902 18.0009L18 1.99991M18 1.99991H3.59912M18 1.99991V16.4008" stroke={text == "light" ? "white" : "black"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </span>
      </div>
    </article>
  );
