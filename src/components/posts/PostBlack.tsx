    ...
    <article className="post-card post-category-top group relative has-line-link-white">
      ...
      <div className={`mt-6 text-white`}>
        ...
        <h3 className="text-2xl text-white leading-relaxed">
          <Link href={`/${post.slug}`} className="link-stretched line-link-el">
            {post.title}
          </Link>
        </h3>

        <ul
          className={`flex flex-wrap items-center gap-3 gap-y-1 uppercase text-sm my-6 text-white/75`}
        >
          <li className="flex items-center">
            {/* <Image
              ...
              height={24}
              width={24}
              className="h-6 w-6 border border-[#ABABAB] rounded-full mr-2 bg-white/5"
            /> */}
            {post.author.name}
          </li>
          <li>•</li>
        ...
        <Link
          href={`/${post.slug}`}
          className="inline-block text-[#90A096] group-hover:text-white group-hover:rotate-45 transition duration-300 mt-4"
